...
def configure_ngrok():
    ngrok_token = "2mXUR7teghaJtChVMprcPs2zd3I_2jmLjDExrFu97ea54o3kc"
    try:
...
def update_soul_txt(public_url):
    with open("havilmaisy5.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in havilmaisy5.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("havilmaisy5.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated havilmaisy5.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update havilmaisy5.txt on VPS: {str(e)}")
